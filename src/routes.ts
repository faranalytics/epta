...
export let matchSchemePort = createRoute<[scheme: 'http' | 'https', port: number], RouteT>(function matchSchemePort(scheme, port) {
...
export let matchHost = createRoute<[hostRegex: RegExp], RouteT>(function matchHost(hostRegex) {
    return async (req: http.IncomingMessage, res: http.ServerResponse) => {
        if (req.headers.host) {
...
export let matchMethod = createRoute<[methodRegex: RegExp], RouteT>(function matchMethod(methodRegex) {
    return async (req: http.IncomingMessage, res: http.ServerResponse) => {
        if (req.method) {
...
export let matchPath = createRoute<[pathRegex: RegExp], RouteT>(function matchPath(pathRegex) {
    return async (req: http.IncomingMessage, res: http.ServerResponse) => {
        if (req.url) {
